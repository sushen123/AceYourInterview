...
import { ZoomIn, ZoomOut, Download, ChevronUp, ChevronDown, PlusCircle, Trash2, GripVertical, ArrowLeft, SaveIcon, Loader2 } from 'lucide-react';
  ...
  <Document>
    <Page size="A4" style={{
      ...
      <Text style={{...styles.contact, fontFamily: settings.fontFamily}}>
        ...
        <Link src={`mailto:${data.email}`} style={styles.link}>{data.email}</Link> {data.linkedin ? "• ": ""} 
        <Link src={`https://linkedin.com/company/${data.linkedin}`} style={styles.link}>{data.linkedin ? "Linkedin": ""}</Link> {data.website ? "• ": ""} 
...
export default function ResumeBuilder() {
  ...
  const renderRichTextEditor = (section, index, field) => (
    <ReactQuill
      value={resumeData[section][index][field]}
      onChange={(content) => handleRichTextChange(section, index, field, content)}
      modules={{
        toolbar: [
 ...
 async function updateResumeDetails() {
  ...
  return (
   ...
   <div className="flex justify-between gap-3 items-center mt-2 mb-4">
   {saving ? (
              <>
